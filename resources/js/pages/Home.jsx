...
import { RevealSection, StaggerChildren } from '@components/AnimatedText';
...
function ParticleCanvas() {
    ...
    useEffect(() => {
        ...
        const getCount = () => {
            ...
            if (w < 640) return 25; if (w < 768) return 35;
            return Math.min(Math.floor((W*H)/18000), 60);
        };
        const pts = Array.from({ length: getCount() }, () => ({
        ...
        const draw = () => {
            ...
            const cd = window.innerWidth < 768 ? 80 : 120;
            ...
            pts.forEach(p => {
                ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fillStyle=`rgba(99,130,255,${p.a})`;ctx.fill();
                p.x+=p.vx; p.y+=p.vy;
    ...
    return <canvas ref={ref} style={{ position:'absolute',inset:0,width:'100%',height:'100%',pointerEvents:'none',opacity:window.innerWidth<768?0.4:0.6,zIndex:2 }}/>;
}

/* ── Marquee (ambil dari SKILLS) ── */
function Marquee() {
...
function Avatar({ isMobile }) {
    const avatarSize = isMobile ? 200 : 280;
    return (
        <div style={{ position:'relative',flexShrink:0 }}>
            {/* Ring luar */}
            <div style={{ position:'absolute',inset:isMobile?'-40px':'-48px',borderRadius:'50%',border:'1px solid rgba(59,110,255,0.12)',animation:'spinSlow 20s linear infinite' }}>
            ...
            {/* Ring dalam */}
            <div style={{ position:'absolute',inset:isMobile?'-28px':'-32px',borderRadius:'50%',border:'1px dashed rgba(249,115,22,0.15)',animation:'spinSlow 14s linear infinite reverse' }}>
            ...
            {/* Foto */}
            <div style={{ position:'relative',width:avatarSize,height:avatarSize,borderRadius:'50%',overflow:'hidden',border:'2px solid rgba(59,110,255,0.35)',boxShadow:'0 0 60px rgba(59,110,255,0.2), 0 0 120px rgba(59,110,255,0.08)',animation:'avatarIn 0.8s cubic-bezier(0.22,1,0.36,1) 0.3s forwards',opacity:0 }}>
                {PROFIL.foto
            ...
            {/* Status badge */}
            <div style={{ position:'absolute',bottom:isMobile?-12:-8,left:'50%',transform:'translateX(-50%)',display:'flex',alignItems:'center',gap:8,padding:isMobile?'6px 16px':'8px 20px',borderRadius:999,whiteSpace:'nowrap',background:'rgba(15,23,42,0.9)',backdropFilter:'blur(12px)',border:'1px solid rgba(255,255,255,0.08)',boxShadow:'0 8px 32px rgba(0,0,0,0.4)' }}>
            ...
            {!isMobile && [
                { label:'Laravel', top:'12%', right:'230px', color:'#7ca0ff', bg:'rgba(59,110,255,0.1)',  border:'rgba(59,110,255,0.22)', delay:'0s'   },
                { label:'React',   top:'44%', right:'270px', color:'#fb923c', bg:'rgba(249,115,22,0.08)', border:'rgba(249,115,22,0.18)', delay:'1.5s' },
                { label:'MySQL',   top:'76%', right:'230px', color:'#a5b4fc', bg:'rgba(99,102,241,0.08)', border:'rgba(99,102,241,0.18)', delay:'3s'   },
            ].map(b => (
                <div key={b.label} style={{ position:'absolute',top:b.top,right:b.right,padding:'6px 14px',borderRadius:8,background:b.bg,border:`1px solid ${b.border}`,backdropFilter:'blur(8px)',color:b.color,fontSize:11,fontWeight:600,fontFamily:'monospace',letterSpacing:'0.05em',animation:`floatBadge 4s ease-in-out ${b.delay} infinite` }}>
                    {b.label}
                </div>
            ))}
        </div>
    ...
    return (
        <>
            <style>{`
                ...
                .h3{animation:heroIn 0.8s cubic-bezier(0.22,1,0.36,1) 0.35s forwards;opacity:0}
                ...
                .btn-primary::before{content:'';position:absolute;inset:0;background:linear-gradient(120deg,transparent 0%,rgba(255,255,255,0.15) 50%,transparent 100%);transform:translateX(-100%);transition:transform 0.5s ease;}
                ...
                @media (max-width: 767px) {
                    ...
                    .section-eyebrow{margin-bottom:12px;font-size:10px;}
            ...
            <section style={{ minHeight:'100vh',display:'flex',alignItems:'center',position:'relative',overflow:'hidden',paddingTop:isMobile?40:80 }}>
                ...
                <div style={{ position:'relative',zIndex:3,maxWidth:1200,margin:'0 auto',padding:isMobile?'40px 16px':'80px 24px',width:'100%' }}>
                    <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',gap:isMobile?0:64,flexWrap:'wrap',flexDirection:isMobile?'column-reverse':'row' }}>
                            ...
                            <div className="h8" style={{ display:'flex',gap:14,flexWrap:'wrap',justifyContent:isMobile?'center':'flex-start' }}>
                                {STATS.map(({ value, label, icon }, i) => (
                                    <div key={i} className="stat-card">
                                        ...
                                        <div style={{ fontFamily:'"Space Grotesk",sans-serif',fontWeight:800,fontSize:30,color:'white',lineHeight:1 }}>{value}</div>
            ...
            {/* ══ PROJECTS ══ */}
            <section style={{ maxWidth:1200,margin:'0 auto',padding:isMobile?'60px 16px':'120px 24px' }}>
                <RevealSection>
                    <div style={{ display:'flex',alignItems:'flex-end',justifyContent:'space-between',marginBottom:isMobile?40:60,gap:24,flexWrap:'wrap' }}>
                        <div>
                            <div className="section-eyebrow">Karya Terbaru</div>
                            <h2 style={{ fontFamily:'"Space Grotesk",sans-serif',fontWeight:800,fontSize:'clamp(32px,4vw,48px)',color:'white',margin:0,lineHeight:1.1 }}>
                ...
                <StaggerChildren className="grid md:grid-cols-3 gap-6" stagger={120}>
                    {featured.map(p => <ProjectCard key={p.id} project={p}/>)}
                </StaggerChildren>
            </section>

            {/* ══ SKILLS ══ */}
            <section style={{ maxWidth:1200,margin:'0 auto',padding:isMobile?'60px 16px':'120px 24px',borderTop:'1px solid rgba(255,255,255,0.04)' }}>
                <RevealSection>
                    <div style={{ display:'flex',alignItems:'flex-end',justifyContent:'space-between',marginBottom:isMobile?40:60,gap:24,flexWrap:'wrap' }}>
                        <div>
                ...
                <StaggerChildren className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4" stagger={80}>
                    {topSkills.map(s => <SkillBadge key={s.id} skill={s}/>)}
                </StaggerChildren>
            </section>

            {/* ══ CTA ══ */}
            <RevealSection>
                <section style={{ padding:isMobile?'0 16px 60px':'0 24px 120px' }}>
                    <div style={{ maxWidth:1200,margin:'0 auto' }}>
                        <div className="cta-box">
                            ...
                            <div style={{ maxWidth:500,position:'relative',textAlign:isMobile?'center':'left' }}>
                                <div className="section-eyebrow" style={{ justifyContent:isMobile?'center':'flex-start' }}>Kolaborasi</div>
                                <h2 style={{ fontFamily:'"Space Grotesk",sans-serif',fontWeight:800,fontSize:'clamp(28px,4vw,42px)',color:'white',margin:'0 0 16px',lineHeight:1.15 }}>
                            ...
                            <Link to="/contact" className="btn-primary" style={{ flexShrink:0,fontSize:isMobile?14:16,padding:isMobile?'12px 28px':'16px 40px' }}>
                                Mulai Diskusi
